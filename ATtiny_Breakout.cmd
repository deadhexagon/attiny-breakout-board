# Pin name action command file

# Start of element S1
ChangePinName(S1, 2, 2)
ChangePinName(S1, 1, 1)
ChangePinName(S1, 3, 3)
ChangePinName(S1, 4, 4)

# Start of element J2
ChangePinName(J2, 4, 4)
ChangePinName(J2, 3, 3)
ChangePinName(J2, 2, 2)
ChangePinName(J2, 1, 1)

# Start of element J3
ChangePinName(J3, 4, 4)
ChangePinName(J3, 3, 3)
ChangePinName(J3, 2, 2)
ChangePinName(J3, 1, 1)

# Start of element C4
ChangePinName(C4, 2, PIN2)
ChangePinName(C4, 1, PIN1)

# Start of element C3
ChangePinName(C3, 2, PIN2)
ChangePinName(C3, 1, PIN1)

# Start of element X1
ChangePinName(X1, 2, 2)
ChangePinName(X1, 1, 1)

# Start of element J1
ChangePinName(J1, unknown, unknown)
ChangePinName(J1, unknown, unknown)
ChangePinName(J1, 3, SCK)
ChangePinName(J1, 4, MOSI)
ChangePinName(J1, 1, MISO)
ChangePinName(J1, 5, \_RESET\_)

# Start of element R1
ChangePinName(R1, 1, 1)
ChangePinName(R1, 2, 2)

# Start of element C2
ChangePinName(C2, 2, -)
ChangePinName(C2, 1, +)

# Start of element C1
ChangePinName(C1, 2, PIN2)
ChangePinName(C1, 1, PIN1)

# Start of element U1
ChangePinName(U1, unknown, unknown)
ChangePinName(U1, unknown, unknown)
ChangePinName(U1, 7, SCK)
ChangePinName(U1, 6, MISO)
ChangePinName(U1, 5, MOSI)
ChangePinName(U1, 3, XTAL2)
ChangePinName(U1, 2, XTAL1)
ChangePinName(U1, 1, \_RESET\_)
